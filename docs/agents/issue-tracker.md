...
> **Setup note:** this repo has no git remote configured yet. `gh` resolves the target repo
> from `git remote -v`, so before these operations work you'll need to create the repo and add
> a remote — e.g. `gh repo create oliverrc/reaparr --source=. --private --push` (or
> `git remote add origin git@github.com:oliverrc/reaparr.git` for an existing repo).
